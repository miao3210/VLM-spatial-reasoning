...
class RoadLayoutDynamicSampler:
    ...
    def _get_label(self, xodr_path, label_group='base'):
        ...
        base_label = {
            'category': 'unknown', 
            'road_type': 'line',   
            'num_roads': len(road_info.get('sorted_road_ids', [])),
            'num_lanes': sum([
                len(road_info['lanes'][k])
                for k in road_info.get('lanes', {}).keys()
            ]),
            'road_length': None,
    ...
    # Miao suggests
    # try to see what is inside get road info from xodr
    # check what is load_xodr
    # and put ot in advanced label
